    ...
    public static Event FromString(string source)
    {
        ...
        switch (sourcePart.Length)
        {
            ...
            case > 10:
            {
                ...
                for (var i = 10; i < sourcePart.Length; i++)
                {
                    text += $",{sourcePart[i]}";
                }
        ...
        return new Event(
            type: typeSplit[0].Replace(':', ' ').Trim(),
            layer: int.TryParse(sourcePart[0], out var result) ? result : 0,
            start: sourcePart[1].Trim(),
            end: sourcePart[2].Trim(),
            style: sourcePart[3].Trim(),
            name: sourcePart[4].Trim(),
            marginL: int.TryParse(sourcePart[5], out result) ? result : 0,
            marginR: int.TryParse(sourcePart[6], out result) ? result : 0,
            marginV: int.TryParse(sourcePart[7], out result) ? result : 0,
            effect: sourcePart[8].Trim(),
